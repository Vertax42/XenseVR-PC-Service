...
if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/RobotDataRecorder" ]; then
        ...
        if [ -f "$LINUX_DIR/RobotDataRecorder" ]; then
            echo "Build successful! Executable is at $LINUX_DIR/RobotDataRecorder"
            ...
            cp "$LINUX_DIR/RobotDataRecorder" "$TARGET_DIR/"
            echo "Executable copied to $TARGET_DIR/RobotDataRecorder"
        else
