...
if [ $? -eq 0 ]; then
    ...
    if [ -f "$TARGET_DIR/ConsoleDemo" ]; then
        echo "Build successful! Executable is at $TARGET_DIR/ConsoleDemo"
    else
        # Check if it was built to the linux directory instead
        LINUX_DIR="$DIR/../../../Redistributable/linux"
        if [ -f "$LINUX_DIR/ConsoleDemo" ]; then
            echo "Build successful! Executable is at $LINUX_DIR/ConsoleDemo"
            ...
            cp "$LINUX_DIR/ConsoleDemo" "$TARGET_DIR/"
            echo "Executable copied to $TARGET_DIR/ConsoleDemo"
        else
            echo "Warning: Executable not found at expected locations"
            echo "Searching for the executable..."
            find $DIR/../../../ -type f -executable -name "ConsoleDemo*"
        fi
    fi
else
    echo "Build failed!"
